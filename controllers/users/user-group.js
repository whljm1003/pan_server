  ...
  sendMail: async (req, res) => {
    ...
    if (email.length === 1) {
      ...
      async function main() {
        ...
        const url = `https://api.picanote.site/invite/?token=${inviteTokenFirst}`;
        //배포 클라이언트 주소로 바꾸기
        ...
        return res.status(200).json({
          groupInfo: groupInfo.dataValues,
          message: "그룹 초대 메일이 발송되었습니다.",
        });
      }
      main(); //이메일 발송 함수 실행
    }
    ...
    if (email.length === 2) {
      ...
      const inviteTokenFirst = jwt.sign(
        { userId: userId1, email: email1, groupId },
        process.env.ACCESS_SECRET,
        { expiresIn: "20m" }
      ); // 초대하려는 사람의 userId, email, groupId 정보가 담긴 토큰 생성.
      const inviteDataFirst = jwt.verify(
      ...
      const inviteTokenSecond = jwt.sign(
        { userId: userId2, email: email2, groupId },
        process.env.ACCESS_SECRET,
        { expiresIn: "20m" }
      );
      const inviteDataSecond = jwt.verify(
        inviteTokenSecond,
        process.env.ACCESS_SECRET
      );
      ...
      async function main() {
        ...
        const url1 = `https://api.picanote.site/invite/?token=${inviteTokenFirst}`;
        ...
        const url2 = `https://api.picanote.site/invite/?token=${inviteTokenSecond}`;
