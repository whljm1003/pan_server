...
module.exports = {
    sendMail: async (req, res) => {
        ...
        const userId = userInfo.dataValues.id // email 초대하려는 사람의 userId(가입된 유저들만 초대 가능).
        ...
        async function main() {
            ...
            const info = await transporter.sendMail({
                ...
                html: "<p>아래 링크를 누르면 그룹으로 초대됩니다.</p>" + `http://localhost:5000/invite/?token=${inviteToken}` // html body,  <a href='http://localhost:3000/invite/?email=" + email + "&token=abcdefg'>그룹초대</a>, `http://localhost:3000/invite/?token=${token}`
            });
